...
`SearchImpl.search()` SHALL load the index page via WikiRepository, call Claude to select relevant slugs, load selected pages via WikiRepository, call Claude to generate an answer. It SHALL return `{ answer, sources }`.
