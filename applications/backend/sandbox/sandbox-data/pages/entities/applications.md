...
category: entities
title: Deployed Applications (Kubernetes)
sources:
  - kustomize
related:
  - config/environment
...
Five microservices deployed to the `harvester` Kubernetes namespace. All use `default-low` PriorityClass. Health check endpoint: `GET /healthy` on port 3000.
...
Downloads historical trade data from Binance Vision and caches to GCS bucket `binance_vision`. NATS only.
...
Orchestrates harvest jobs. Receives commands via NATS, publishes to Kafka, persists job state in Postgres (`harvester` DB), runs BigQuery analytics pipeline.
...
Resources: 125m/256Mi req → 1150m/512Mi limit. Mounts GCP key at `/etc/secrets/key.json`.
...
Batch-processes raw trade records. Reads from Kafka, streams trades via NATS to GCS `trades_jsonl`, loads to BigQuery. Uses Kafka transactions (transactional ID from pod name for uniqueness per replica).

| Env | Value |
|-----|-------|
| `NATS_QUEUE` | `harvester-reaper` |
| `TRANSPORTS` | `nats,kafka` |
| `KAFKA_TRANSACTIONAL_ID` | pod name |
...
Serves NASDAQ trade data from GCS bucket `nasdaq-trades`. NATS only.
...
All images tagged via `kustomize edit set image` in `base/applications/kustomization.yaml`. Current deploy tag: `v90` (set via `$TAG` in `deploy.sh`).
